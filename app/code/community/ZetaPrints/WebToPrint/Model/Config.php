...
class ZetaPrints_WebToPrint_Model_Config extends Mage_Core_Model_Config_Base
{
    ...
    private function _prepareOptions($options)
    {
        ...
        foreach ($options as $name => $suboptions) //If sub option is array and is not array of tag attributes...
        {
            if ($name !== '@' && is_array($suboptions)) //... the run the function for the sub-option
            {
