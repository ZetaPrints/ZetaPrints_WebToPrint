    ...
    public function parse()
    {
        ...
        foreach ($catalogs as $catalog) {
            ...
            foreach ($templates as $template) {
                ...
                if (!$template_array) {
                    $this->error(
                        "{$line}. Error in receiving or parsing detailes of template {$template['guid']}. Leaving the template unmodified."
                    );
                    continue;
                }
                ...
                if ($templates_collection->getSize() == 1) {
                    foreach ($templates_collection as $template_model) {
                        if (!$has_fields) {
                            ...
                            $this->warning(
                                "{$line}. Template {$template['guid']} has no variable fields. Template will be deleted."
                            );
                            continue;
                        }
                        ...
                        if (strtotime($template['date']) > strtotime($template_model->getDate())
                            ...
                            if (!$template['xml']) {
                                $this->error(
                                    "{$line}. Error in receiving detailes for template {$template['guid']}. Leaving the template unmodified."
                                );
                                continue;
                            }
                    ...
                    if (!$template['xml']) {
                        $this->error(
                            "{$line}. Error in receiving detailes for template {$template['guid']}. Passing the template."
                        );
        ...
        if ($number_of_added_templates > 0) {
            $this->notice(
                'Run ZetaPrints product creation profile to create products for new templates automatically. Read <a href="http://www.zetaprints.com/magentohelp/">www.zetaprints.com/magentohelp/</a> for more info on product creation.'
            );
        }
    }
