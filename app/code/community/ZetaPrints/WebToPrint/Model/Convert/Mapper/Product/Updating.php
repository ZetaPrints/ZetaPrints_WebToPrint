    ...
    public function map()
    {
        ...
        foreach ($templates as $template) {
            ...
            if ($product_id = $product_model->getIdBySku($template->getGuid())) {
                ...
                $products = $product_model->getCollection()->addAttributeToFilter(
                    'webtoprint_template',
                    ['eq' => $template->getGuid()]
                )->load();
                    ...
                    if ($behaviour == ZetaPrints_WebToPrint_Model_System_Config_Source_TemplateDeletingBehaviour::NONE) {
                        ...
                        if ($behaviour >= 0) {
                            ...
                            foreach ($products as $product) {
                                ...
                                $this->debug(
                                    "Template for product {$full_product->getWebtoprintTemplate()} was removed"
                                );

                                Mage::register('webtoprint-template-changed', true);
                                $full_product
                                    ->setCategoryIds([$behaviour])
                                    ->setRequiredOptions(false)
                                    ->setWebtoprintTemplate('');
                                ...
                                $this->debug(
                                    "Product {$product->getSku()} was moved to category {$category->getName()}"
                                );
                            }
