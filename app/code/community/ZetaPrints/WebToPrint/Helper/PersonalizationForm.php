    ...
    implements ZetaPrints_Api
{
    ...
    private function get_form_part_html($form_part = null, $product, $params = [])
    {
        ...
        if (file_exists($locale_file) || file_exists($custom_translations_file)) {
            ...
            if (strlen($out) == 0) {
                ...
                if (is_array($array) && count($array[1]) > 0) {
                    ...
                    $out .= "</trans>";
                    $cache->save($out, "XMLTranslation" . Mage::app()->getLocale()->getLocaleCode(), ['TRANSLATE']);
                }
            }
...
CODE;
  ...
  <input id="zetaprints-share-link-input" type="text" value=""/>
        ...
        foreach ($xml->Images->Image as $image_node) {
            if (isset($image_node['AllowUpload'])) {
                foreach ($images as $image) {
                    ...
                    $user_image_node->addAttribute(
                        'edit-link',
                        $this->_getUrl(
                            'web-to-print/image/',
                            ['id' => $image['guid'], 'iframe' => 1]
                        )
                    );
                }
            }
    ...
    public function get_order_webtoprint_links($context, $item = null)
    {
        ...
        if (!isset($options['zetaprints-TemplateID'])) //... just return from the function.
        {
            return;
        }
        ...
        if ($isAdmin && !$isOrderComplete) {
            $url = Mage::helper('adminhtml')
                ->getUrl(
                    'adminhtml/webtoprint_order/complete',
                    ['item' => $item->getId()]
                );
        ...
        $types = ['pdf', 'gif', 'png', 'jpeg'];

        //If function called from admin template
        if ($isAdmin) //then add CDR file type to list of available types
        {
    ...
    public function get_order_preview_images($context, $item = null)
    {
        ...
        <tr class="border zetaprints-previews">
            <td class="last" colspan="<?php echo $item ? 5 : 10; ?>">
                <div class="zetaprints-previews-box <?php if ($item) {
                    echo 'hidden';
                } ?>">
                    <div class="title">
                        ...
                        <ul>
                            <?php foreach ($previews as $preview): ?>
                                <li>
                                    ...
                                    <?php else: ?>
                                        <a class="in-dialog" href="<?php echo $this->get_preview_url($preview); ?>"
                                           target="_blank" rel="<?php echo $group; ?>">
    ...
    public function get_reorder_button($context, $item)
    {
        ...
        //If it doesn't exist or product doesn't have web-to-print features then...
        if (!isset($options['zetaprints-order-id'])) //... just return from the function.
        {
            return;
        }
        ...
        $url = $product->getUrlInStore(
            [
                '_query'
                => ['reorder' => $options['zetaprints-order-id']],
            ]
        );
    ...
    public function getPageSizeTable($context, $units = 'in')
    {
        ...
        $result = $this->get_form_part_html(
            'page-size-table',
            $context->getProduct(),
            $params
        );
            ...
            <?php foreach ($dataset as $pageNumber => $_dataset): ?>
                <div id="zp-dataset-page-<?php echo $pageNumber; ?>" class="zp-dataset">
                    ...
                    <p class="zp-dataset-notice">
                        <?php echo $this->__(
                            '* Click on a cell to insert the value or click on the first column to insert the entire row.'
                        ); ?>
                    </p>
                </div>
            <?php endforeach; ?>
    ...
    public function getJs($context)
    {
        ...
        <script type="text/javascript">
            ...
            jQuery(document).ready(function ($) {
                ...
                cannot_update_preview_second_time = "<?php echo $this->__(
                    'Cannot update the preview. Try again or add to cart as is and we will update it manually.'
                ); ?>";
                preview_sharing_link_error_text = "<?php echo $this->__(
                    'Error was occurred while preparing preview image'
                ); ?>";
                ...
                notice_to_update_preview_text_for_multipage_template = "<?php echo $this->__(
                    'Update all previews first!'
                ); ?>";
                warning_user_data_changed = "<?php echo $this->__(
                    'Press Update Preview before adding to cart to include your latest changes'
                ); ?>";
                notice_update_preview_after_data_changed = "<?php echo $this->__(
                    'Do you want to update the preview with your latest changes?'
                ); ?>";
